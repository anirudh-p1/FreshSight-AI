...
is to the end of its shelf life. The three automated actions are: 
1 – continued sale at full price,
2 – an AI optimised price reduction to accelerate purchase, and 
3 – an automated alert sent
...
needed to ensure the item sells before expiry, which is based on historical sales data for that product. This prevents over-discounting, which reduces revenue for the firms, and under-
discounting, which produces waste. When an item’s score falls below the threshold for foodbanks, the system sends an automated
